...
extern(C) @nogc nothrow {
	...
	alias psoundio_channel_layout_find_channel = int function(
		const(SoundIoChannelLayout)* layout, SoundIoChannelId channel
	);
	alias psoundio_channel_layout_detect_builtin = bool function(SoundIoChannelLayout* layout);
	alias psoundio_best_matching_channel_layout = const(SoundIoChannelLayout)* function(
		const(SoundIoChannelLayout)* preferred_layouts, int preferred_layout_count,
		const(SoundIoChannelLayout)* available_layouts, int available_layout_count
	);
	...
	alias psoundio_outstream_begin_write = int function(
		SoundIoOutStream* outstream, SoundIoChannelArea** areas, int* frame_count
	);
	...
	alias psoundio_instream_begin_read = int function(
		SoundIoInStream* instream, SoundIoChannelArea** areas, int* frame_count
	);
 ...
 * free memory or reload a new version, this function should be used.
...
/**
 * Load a libsoundio dynamic library by automatically trying common names for the library.
 *
 * If you want to specify which dynamic library should be
 * loaded, refer to the overload that takes a libName parameter.
 *
 * Returns: loaded version or error code
 */
...
LibsoundioSupport loadLibsoundio(const(char)* libName) {
	...
	if (errorCount() != errCount) {
		return LibsoundioSupport.badLibrary;
	}
