...
static if (libsoundioSupport >= LibsoundioSupport.libsoundio11) {
    ...
    /// Returns: string representation of this libsoundio version, as `major.minor.patch`
	/// See_Also: [soundio_version_major], [soundio_version_minor], [soundio_version_patch]
	const(char)* soundio_version_string();

	/// See_Also: [soundio_version_string]
...
///
/// Possible errors:
/// - Invalid - already connected or invalid backend parameter
/// - NoMem
/// - BackendUnavailable - backend was not compiled in
/// - SystemResources
/// - NoSuchClient - when JACK returns `JackNoSuchClient`
/// - InitAudioBackend - requested `backend` is not active
/// - BackendDisconnected - backend disconnected while connecting
/// See_Also: [soundio_disconnect]
...
///
/// Note that calling this function merely flips a pointer;
/// the actual work of collecting device information is done elsewhere.
...
/// is ready or you call [soundio_wakeup].
///
...
/// Params:
///   id = a sound channel id
/// Returns: UTF-8 encoded text which is the name of a channel such as "Front Left", "FL", or "front-left"
const(char)* soundio_get_channel_name(SoundIoChannelId id);

/// Returns: the corresponding `SoundIoChannelId` or `SoundIoChannelId.Invalid` for no match.
...
/// Params:
///   index = index in range 0..[soundio_channel_layout_builtin_count]
...
/// Returns: the first channel layout in
...
/// Params:
///   layouts = pointer to first element in list
///   layout_count = length of `layouts`
...
/// Returns: the amount of bytes a single frame takes for the given format
int soundio_get_bytes_per_frame(SoundIoFormat format, int channel_count);

/// Sample rate is the number of frames per second.
/// Returns: bytes per second
int soundio_get_bytes_per_second(
...
/// When you want to check for new devices, call [soundio_flush_events].
/// Or you can call [soundio_wait_events] to block until devices change.
/// If an error occurs scanning devices in a background thread, [SoundIo.on_backend_disconnect] is called with the error code.
...
/// Returns `null` if and only if memory could not be allocated.
/// See_Also: [soundio_outstream_destroy
...
static if (libsoundioSupport >= LibsoundioSupport.libsoundio20) {
    /// Set the volume of an OutStream
...
/// Returns: `null` if and only if memory could not be allocated.
/// See_Also: [soundio_instream_destroy]
...
/// See_Also: [soundio_ring_buffer_destroy]
...
/// capacity for alignment purposes.
/// This function returns the actual capacity.
/// Returns: actual capacity of the ring buffer in bytes
...
/// Returns: a pointer where you can write to
...
/// Clear all data in the ring buffer, making fill count equal to 0 and free count equal to the maximum.
///
